...
评价指标方面，参考 `HBHD_predict_v1.5/predict_check.py` 中的 `calculate_metrics`，在 `src/tsas/engine/operator/evaluation` 下新增 `ForecastingMetrics` 算子，统一提供 MAE、RMSE、MAPE、DTW 四项指标。
